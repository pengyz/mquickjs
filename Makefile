...
# 添加新工具到程序列表
MODULE_GEN=mqjs_js_module_gen

all: $(PROGS) $(MODULE_GEN)
...
mqjs_stdlib: examples/mquickjs_build.host.o src/repl/mqjs_stdlib.host.o
	$(HOST_CC) $(HOST_LDFLAGS) -o $@ $^

examples/mquickjs_build.host.o: examples/mquickjs_build.c
	$(HOST_CC) $(HOST_CFLAGS) -Isrc/utils -Isrc/core -c -o $@ $<

src/repl/mqjs_stdlib.host.o: src/repl/mqjs_stdlib.c
	$(HOST_CC) $(HOST_CFLAGS) -Isrc/utils -Isrc/core -Iexamples -c -o $@ $<
...
examples/example_stdlib: examples/example_stdlib.host.o examples/mquickjs_build.host.o
	$(HOST_CC) $(HOST_LDFLAGS) -o $@ $^

examples/example_stdlib.host.o: examples/example_stdlib.c
	$(HOST_CC) $(HOST_CFLAGS) -Isrc/utils -Isrc/core -Iexamples -Isrc/repl -c -o $@ $<

examples/example_stdlib.h: examples/example_stdlib
	./examples/example_stdlib $(MQJS_BUILD_FLAGS) > $@

# Module generator tool
mqjs_js_module_gen: tools/mqjs_js_module_gen.o src/utils/cutils.o
	$(CC) $(LDFLAGS) -o $@ $^ $(LIBS)
...
%.host.o: %.c
	$(HOST_CC) $(HOST_CFLAGS) -Isrc/utils -Isrc/core -Iexamples -Isrc/libc -c -o $@ $<
...
.PHONY: clean
clean:
	rm -f *.o *.d *~ tests/*.o tests/*.d tests/*~ test_builtin.bin mqjs_stdlib mqjs_stdlib.h mquickjs_build_atoms mquickjs_atom.h mqjs_example example_stdlib example_stdlib.h $(PROGS) $(TEST_PROGS) $(MODULE_GEN)
	find . \( -name "*.o" -o -name "*.d" \) -type f -not -path "./build/*" -delete
